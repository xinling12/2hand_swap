...
<div class="container cart">
	...
	<!-- Cart Totals -->
	<div class="sixteen columns">
		<h3 class="headline">My Wishlist On 2hand Swap</h3><span class="line margin-bottom-35"></span><div class="clearfix"></div>
	</div>
	...
	<div class="sixteen columns">
		...
		<table class="cart-table responsive-table">
			...
			$sql = "select * from wishlist where email='".$username."';";
			...
			while($row = mysqli_fetch_array($result)){
				...
				$stock = $row['stock'];
				$pro_id = $row['pro_id'];
				echo"			<form action='deletewish.php' method='get'
								<tr>
									...
									<td><a href='deletewish.php?pro_id=".$pro_id."' class='cart-remove'></a></td>
								</tr>
								</form>";
			}
			?>
			...
			$sql_sum = "select sum(stock*price) as total from wishlist where email='".$_SESSION['email']."';";
			$result1 = $conn->query($sql_sum);
			while($row = mysqli_fetch_array($result1)){
				$total = $row['total'];
				echo "				<th>".$total." AUD</th>";
			}
			$db->disconnect();
			?>
				<th></th>
